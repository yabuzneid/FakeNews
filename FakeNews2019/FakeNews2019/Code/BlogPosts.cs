using FakeNews2019.DB;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Web;
using System.Xml.Serialization;

namespace FakeNews2019.Code
{
    
    public class BlogPostsProvider
    {
        public   List<FakePost> BlogPosts { get; set; }

        private static BlogPostsProvider current;

        private static BlogPostsProvider Current
        {
            get
            {
                //    if (current == null)
                //    {
                using (var ctx = new FakeNews2019Entities())
                {
                    var res = (from s in ctx.FakePosts

                               select s).ToList();
                    BlogPostsProvider blogPostsProvider = new BlogPostsProvider();
                    blogPostsProvider.BlogPosts = res;
                }
                 
                string filePath = HttpContext.Current.Server.MapPath("~/App_Data/BlogPosts.xml");
                using (StreamReader reader = new StreamReader(filePath))
                {
                    XmlSerializer serializer = new XmlSerializer(typeof(BlogPostsProvider));
                    current = (BlogPostsProvider)serializer.Deserialize(reader);
                }
                // }
                return current;
            }
        }

        public static FakePost GetBlogPost(int? id)
        {
            if (!id.HasValue)
                return null;
            return Current.BlogPosts.FirstOrDefault(p => p.Id == id);
        }

        public static List<FakePost> GetBlogPosts()
        {
            return GetBlogPosts(null, null);
        }

        public static List<FakePost> GetBlogPosts(int? year, int? month)
        {
            List<FakePost> blogPosts = Current.BlogPosts;
            if (blogPosts == null)
                return null;

            if (year.HasValue)
                blogPosts = blogPosts.FindAll(p => p.Date.Year == year);
            if (month.HasValue)
                blogPosts = blogPosts.FindAll(p => p.Date.Month == month);
            return blogPosts;
        }

        public static Dictionary<int, Dictionary<int, IEnumerable<FakePost>>> GetBlogsByCategories()
        {
            var result = new Dictionary<int, Dictionary<int, IEnumerable<FakePost>>>();
            var yearsGroup = Current.BlogPosts.GroupBy(p => p.Date.Year).OrderByDescending(g => g.Key);
            foreach (var yearGroup in yearsGroup)
            {
                result[yearGroup.Key] = new Dictionary<int, IEnumerable<FakePost>>();
                var monthGroups = yearGroup.GroupBy(p => p.Date.Month).OrderByDescending(g => g.Key);
                foreach (var monthGroup in monthGroups)
                {
                    result[yearGroup.Key][monthGroup.Key] = monthGroup.ToList();
                }
            }
            return result;
        }
    }

    //public class BlogPost
    //{
    //    public int Id { get; set; }
    //    public string Category { get; set; }
    //    public string Subject { get; set; }
    //    public string Body { get; set; }
    //    public string ImageUrl { get; set; }

    //    [XmlIgnore]
    //    public DateTime Date { get; set; }
    //    public string DateString
    //    {
    //        get { return this.Date.ToString("yyyy-MM-dd HH:mm:ss"); }
    //        set { this.Date = DateTime.Parse(value); }
    //    }
    //}
}